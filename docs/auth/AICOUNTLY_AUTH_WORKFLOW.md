# AICOUNTLY auth workflow (Messaging)

How Messaging signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
| Messaging host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `messaging.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `messaging.aicountly.com/api` |
| `messaging.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `messaging.gh.aicountly.com/api` |
...
`messaging` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://messaging.gh.aicountly.com/api/health
...
curl -i -X POST https://messaging.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://messaging.gh.aicountly.com/api/global/login   # expect 404
```
