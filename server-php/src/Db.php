 ...
 *
 * It reaches exactly one schema: the tables Messaging owns — channels,
 * conversations, messages, delivery events, templates, journeys, consent. It is
 * never pointed at Contacts, Books, Sales, Pay or Manage, and there is no
 * second connection, no foreign data wrapper and no `dblink` anywhere in this
 * product. Those products are read and written through their HTTP APIs (see
 * src/Clients).
 *
 * A cross-database join is not a shortcut here, it is the architecture failing:
 * two products would then hold two answers to the same question — what does
 * this customer owe? — and something would have to reconcile them. A message
 * this product stored quotes a balance as it stood when it was sent; Books says
 * what it is now. Only one of those is authoritative and it is not this one.
 */
final class Db
{
