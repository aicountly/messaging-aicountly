...
final class Router
{
    ...
    /**
     * The registered table, for a test that checks every handler exists.
     *
     * Read-only and derived. A route declared against a method somebody
     * renamed is a 500 the first time a customer opens that screen, and this is
     * what makes it a failing test instead.
     *
     * @return list<array{method:string, path:string, handler:callable}>
     */
    public function routes(): array
    {
        return array_map(static fn (array $route) => [
            'method'  => $route['method'],
